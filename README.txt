...
The source folder contains three ipynb files:
    ...
    nltk, sklearn, collections, scipy).  If you're just hoping to view the notebook (with visualizations), try this link:
...
The data.zip file contains three files that were used in analysis:
    ...
    labels.  0 indicates that a sentence does not need to be simplified while 1 indicates that a sentence does need to be simplified.
    
    dale_chall.txt:  This is the Dale Chall 3000 Word List, which is one definition of words that are considered
    "basic" English.  A summary is at https://www.readabilityformulas.com/articles/dale-chall-readability-word-list.php
    
    AoA_51715_words.csv:  This file contains "Age of Acquisition" (AoA) estimates for about 51k English words,
    which refers to the approximate age (in years) when a word was learned. Early words, being more basic, have lower average AoA.  The main columns you will be interested in (utilized in the project) are
